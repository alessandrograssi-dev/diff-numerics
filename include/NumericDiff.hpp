...
namespace numdiff {
    ...
    private:
        ...
        size_t FilesColumns(const std::string& file) const;
        // Helper: check if a line is a comment
        inline bool isLineComment(const std::string& line) const {
            size_t pos = line.find_first_not_of(" \t");
            if (pos == std::string::npos) return false;
            return line.compare(pos, options_.comment_prefix.size(), options_.comment_prefix) == 0;
        }
        ...
        inline bool lineContainsComment_but_isNotJustComment(const std::string& line) const {
            size_t pos = line.find(options_.comment_prefix);
        ...
        std::pair<bool, double> compareLine(const std::string& line1, const std::string& line2) const;
